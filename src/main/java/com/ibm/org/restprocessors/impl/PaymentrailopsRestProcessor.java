...
@Service
public class PaymentrailopsRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PaymentrailopsRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.paymentrailops.hosturl}")
	private String url;
	
	@Value("${spring.api.paymentrailops.context-path}")
	private String contextPath;

	@Value("${spring.api.paymentrailops.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PaymentrailopsRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PaymentrailopsRestProcessor.class.getName());
