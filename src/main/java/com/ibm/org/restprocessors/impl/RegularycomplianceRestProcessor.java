...
@Service
public class RegularycomplianceRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(RegularycomplianceRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.regularycompliance.hosturl}")
	private String url;
	
	@Value("${spring.api.regularycompliance.context-path}")
	private String contextPath;

	@Value("${spring.api.regularycompliance.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  RegularycomplianceRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ RegularycomplianceRestProcessor.class.getName());
