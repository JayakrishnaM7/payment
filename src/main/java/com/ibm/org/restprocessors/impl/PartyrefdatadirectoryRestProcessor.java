...
@Service
public class PartyrefdatadirectoryRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PartyrefdatadirectoryRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.partyrefdatadirectory.hosturl}")
	private String url;
	
	@Value("${spring.api.partyrefdatadirectory.context-path}")
	private String contextPath;

	@Value("${spring.api.partyrefdatadirectory.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PartyrefdatadirectoryRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PartyrefdatadirectoryRestProcessor.class.getName());
