...
@Component
public class Conductchecks implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private RegularycomplianceRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Conductchecks.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Conductchecks.class.getName());
         }else{
